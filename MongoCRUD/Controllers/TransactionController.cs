...
public class TransactionController(DbContext db) : ControllerBase
{
    ...
    public async Task Demo()
    {
        ...
        try
        {
            ...
            await db.Cat.InsertManyAsync(session, cats);
            await db.Dog.InsertManyAsync(session, dogs);
            _ = await db.Cat.UpdateManyAsync(session, _cbf.Gt(c => c.No, 50), _cbu.Set(c => c.Name, "Tom"));
            _ = await db.Dog.UpdateManyAsync(session, _dbf.Gt(c => c.No, 50), _dbu.Set(c => c.Name, "Spike"));
            _ = await db.Cat.DeleteManyAsync(session, c => c.No < 10);
            _ = await db.Dog.DeleteManyAsync(session, c => c.No < 10);
            // 完成事务的操作后提交事务.
    ...
    public async Task WithError()
    {
        ...
        try
        {
            // 这里记住一定要开始事务,不然也不行.
            session.StartTransaction();
            _ = await db.Cat.UpdateManyAsync(session, _cbf.Lte(c => c.No, 50), _cbu.Set(c => c.Name, "Spike"));
            _ = await db.Dog.UpdateManyAsync(session, _dbf.Lte(c => c.No, 50), _dbu.Set(c => c.Name, "Tom"));
            _ = await db.Cat.DeleteManyAsync(session, _ => true);
            _ = await db.Dog.DeleteManyAsync(session, _ => true);
            throw new("error");
